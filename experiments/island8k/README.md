...
the thresholds in `suitability.json`; this experiment merely labels the
selection criterion `village_site` (ADR 0011: curation, not a gate).
...
keep those whose terrain gates pass and whose `village_site` criterion is
satisfied, pick the best.
...
# select: highest-ranked run with OVERALL PASS and a satisfied village_site
# criterion, largest patch area as tie-break; then eyeball the map pack.
```
